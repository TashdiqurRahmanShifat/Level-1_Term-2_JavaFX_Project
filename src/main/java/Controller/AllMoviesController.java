...
public class AllMoviesController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        aLLmovie();
        for(Cinemas search:allmovie)
        {
            table.getItems().add(search);
        }
    ...
    public static void aLLmovie()
    {
        ...
        for (int i=0;i<HelloApplication.cinemasList.size();i++)
        {
           Cinemas geTMovie=HelloApplication.cinemasList.get(i);
            if(geTMovie.getProductioncompany().equalsIgnoreCase(WindowController.username))
               allmovie.add(geTMovie);
        }
    }
}
